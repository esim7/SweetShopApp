...
namespace WebApp.App_Start
{
    ...
    public static class NinjectWebCommon 
    {
        ...
        private static void RegisterServices(IKernel kernel)
        {
            ...
            kernel.Bind<IRepository<OrderDetail>>().To<OrderDetailsRepository>();
            kernel.Bind<IUnitOfWork>().To<EFUnitOfWork>();
            kernel.Bind<SweetShopDataContext>().ToSelf().InSingletonScope();
            kernel.Bind<IMapper>().ToConstant(MapperConfig.GetMapper()).InSingletonScope();
        }
    }
}